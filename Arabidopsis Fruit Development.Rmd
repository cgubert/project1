...
Both NEV and EVR promote fruit growth, as indicated by reduction in fruit length in both mutants. Compared to WT, *nev* fruit are `r ((means["nev"])/(means["WT"])*100)`% as long (`r (means[["nev"]])` ± `r stDevs[2]`mm) and *evr* fruit are `r ((means["evr"])/(means["WT"])*100)`% as long (`r means[["evr"]]` ± `r stDevs[3]`mm). The reduction of growth in the double mutant is significantly greater than either of the individual mutants, supporting the hypothesis that NEV and EVR independently effect fruit growth. *nev evr* fruit are `r ((means["nevevr"])/(means["WT"])*100)`% as long as WT fruit (`r (means[["nev"]])` ± `r stDevs[4]`mm). 

```r
#what are the magnitudes of the differences?
((means["nevevrhae"])/(means["WT"])*100)
((means["nevevrhaehsl"])/(means["WT"])*100)
((means["nevidaevr"])/(means["WT"])*100)
```
Loss of NEV or EVR activity reduces the length of *Arabidopsis* fruit by 
```r ((means["nev"])/(means["WT"])*100)```
% and 
``` r ((means["evr"])/(means["WT"])*100)```
%, repectively. 
######Summary
